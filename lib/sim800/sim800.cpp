// Библиотека для работы с GSM/GPRS SIM800 (основана на библиотеке GSM/GPRS Shield A6, http://iarduino.ru)

#include "sim800.h"																																	        //	
#include "SoftwareSerial.h"																																	//	
																																							//	
//		КОНСТРУКТОР КЛАССА:																																	//	
		sim800::sim800(uint8_t pin){pinPWR=pin;}																								            //	Аргументы конструктора:	pin - номер вывода Arduino к которому подключён вывод PWR.
																																							//	
//		ФУНКЦИЯ ИНИЦИАЛИЗАЦИИ МОДУЛЯ:																														//	Функция возвращает:	флаг результата инициализации (true/false).
bool	sim800::_begin(void){																															    //	Аргументы функции:	отсутствуют.
...
void	sim800::_SMSdecodGSM(char* txt, uint16_t len, uint16_t pos, uint16_t udh_len){																//	Аргументы функции:	txt - строка для текста, len - количество символов в тексте, pos - позиция начала текста в строке strBuffer, udh_len количество байт занимаемое заголовком.
...
//uint16_t sim800::_SMScoder8BIT(String txt, uint16_t pos, uint16_t len){																			//	Аргументы функции:	txt - строка с текстом, pos - позиция взятия первого символа из строки txt, len - количество кодируемых символов из строки txt.
...
void	sim800::_SMSdecod8BIT(char* txt, uint16_t len, uint16_t pos){																				//	Аргументы функции:	txt - строка для текста, len - количество байт в тексте, pos - позиция начала текста в строке strBuffer.
...
void	sim800::_SMSdecodUCS2(char* txt, uint16_t len, uint16_t pos){																				//	Аргументы функции:	txt - строка для текста, len - количество байт в тексте, pos - позиция начала текста в строке strBuffer.
...
String	sim800::runAT(String command, uint32_t timeout, bool early){																				//	Аргументы функции:	command - строка с АТ-командой,
...
String	sim800::runUSSD(String command, uint32_t timeout){																							//	Аргументы функции:	command - строка с USSD-командой,
...
void	sim800::pwr(bool f){																														//	Аргументы функции:	f - флаг (0-выключить / 1-включить)
...
uint8_t	sim800::status(void){																														//	Аргументы функции:	отсутствуют.
...
bool	sim800::pin(const char* pin_code){																											//	Аргументы функции:	pin_code - строка с pin-кодом.
			runAT(F("AT+CPIN?\r\n"));																														//	Выполняем команду проверки аутентификации SIM-карты.
...
bool	sim800::puk(const char* puk_code, const char* pin_code){																					//	Аргументы функции:	puk_code - строка с puk-кодом, pin_code - строка с новым pin-кодом.
			runAT(F("AT+CPIN?\r\n"));																														//	Выполняем команду проверки аутентификации SIM-карты.
...
uint8_t	sim800::signal(void){																														//	Аргументы функции:	отсутствуют.
...
uint8_t	sim800::SMSmax(void){																														//	Аргументы функции:	отсутствуют.
			uint8_t i, j=0;																																	//	Объявляем временные переменные.
			runAT(F("AT+CPMS?\r\n"));																														//	Выполняем команду получения выбранных областей памяти для хранения SMS сообщений.
			if( strBuffer.indexOf( F("+CPMS:") ) < 0){return 0;}																							//	Возвращаем 0, так как в ответе нет текста "+CPMS:".
			i = strBuffer.indexOf( F("+CPMS:") ); 																											//	Получаем позицию начала текста "+CPMS:" в ответе "ЭХО\r\n+CPMS: "ПАМЯТЬ1",ИСПОЛЬЗОВАНО,ОБЪЁМ, "ПАМЯТЬ2",ИСПОЛЬЗОВАНО,ОБЪЁМ, "ПАМЯТЬ3",ИСПОЛЬЗОВАНО,ОБЪЁМ\r\n\r\nOK\r\n".
...
bool	sim800::SMSsend(const char* txt, const char* num, uint16_t lngID, uint8_t lngSum, uint8_t lngNum){											//	Аргументы функции:	txt - передаваемый текст, num - номер получателя, lngID - идентификатор склеенных SMS, lngSum - количество склеенных SMS, lngNum - номер данной склеенной SMS.
//			Готовим переменные:																																//	
...
uint8_t	sim800::CALLstatus(void){																													//	Аргументы функции:	отсутствуют.
            int i,j,k; 
...
bool	sim800::CALLavailable(char* num){																											//	Аргументы функции:	num - указатель для возврата адреса (номера) вызывающего абонента.
...
void	sim800::CALLup(void){																														//	Аргументы функции:	отсутствуют.
...
void	sim800::CALLend(void){																														//	Аргументы функции:	отсутствуют.
...
bool	sim800::CALLdial(const char* num){																											//	Аргументы функции:	num - строка с номером вызываемого абонента
...
uint8_t	sim800::SOUNDdevice(uint8_t device){																										//	Аргументы функции:	device - выбираемое устройство для ввода и вывода звука GSM_HEADSET / GSM_SPEAKER / GSM_MICROPHONE
			int i;																																			//	Объявляем временную переменную.
...
uint8_t	sim800::SOUNDvolume(uint8_t volume){																										//	Аргументы функции:	volume - громкость от 0-нет до 7-макс / без аргументов - получить значение громкости.
			int i;																																			//	Объявляем временную переменную.
...
bool	sim800::SOUNDmute(bool f){																													//	Аргументы функции:	f - флаг true-отключить микрофон / false-не отключать микрофон.
...
bool	sim800::SOUNDmute(void){																													//	Аргументы функции:	отсутствуют.
			...
			return (strBuffer.charAt(i)=='1')?true:false;																									//	Возвращаем флаг наличия немого режима (mute).
																																							...
																																							//	
//		ФУНКЦИИ ДУБЛЁРЫ:																																	//	
String	sim800::runAT			(const char* command, uint32_t timeout, bool early)									{return runAT  (String(command), timeout, early);}
String	sim800::runUSSD		(const char* command, uint32_t timeout            )									{return runUSSD(String(command), timeout       );}
bool	sim800::pin			(                      String      pin_code)										{                    char _pin_code[13];                                      pin_code.toCharArray(_pin_code, 13); return pin(           _pin_code);}
bool	sim800::puk			(String      puk_code, const char* pin_code)										{char _puk_code[13];                     puk_code.toCharArray(_puk_code, 13);                                      return puk(_puk_code,  pin_code);}
bool	sim800::puk			(const char* puk_code, String      pin_code)										{                    char _pin_code[13];                                      pin_code.toCharArray(_pin_code, 13); return puk( puk_code, _pin_code);}
bool	sim800::puk			(String      puk_code, String      pin_code)										{char _puk_code[13]; char _pin_code[13]; puk_code.toCharArray(_puk_code, 13); pin_code.toCharArray(_pin_code, 13); return puk(_puk_code, _pin_code);}
bool	sim800::SMSread		(char* txt)																			{uint16_t _lngID; uint8_t _lngSUM, _lngNUM; char _tim[18], _num[17]; return SMSread(txt,_num,_tim,_lngID,_lngSUM,_lngNUM);}
bool	sim800::SMSread		(char* txt, char* num)																{uint16_t _lngID; uint8_t _lngSUM, _lngNUM; char _tim[18];           return SMSread(txt, num,_tim,_lngID,_lngSUM,_lngNUM);}
bool	sim800::SMSread		(char* txt, char* num, char* tim)													{uint16_t _lngID; uint8_t _lngSUM, _lngNUM;                          return SMSread(txt, num, tim,_lngID,_lngSUM,_lngNUM);}
bool	sim800::SMSsend		(String      txt, const char* num, uint16_t lngID, uint8_t lngSum, uint8_t lngNum)	{char _txt[161];                txt.toCharArray(_txt, 161);                            return SMSsend(_txt,  num, lngID, lngSum, lngNum);}
bool	sim800::SMSsend		(const char* txt, String      num, uint16_t lngID, uint8_t lngSum, uint8_t lngNum)	{                char _num[15];                             num.toCharArray(_num, 15); return SMSsend( txt, _num, lngID, lngSum, lngNum);}
bool	sim800::SMSsend		(String      txt, String      num, uint16_t lngID, uint8_t lngSum, uint8_t lngNum)	{char _txt[161]; char _num[15]; txt.toCharArray(_txt, 161); num.toCharArray(_num, 15); return SMSsend(_txt, _num, lngID, lngSum, lngNum);}
bool	sim800::CALLavailable	(void)																				{char _num[17]; return CALLavailable(_num);}
bool	sim800::CALLdial		(String num)																		{char _num[17]; num.toCharArray(_num, 17); return CALLdial(_num);}
